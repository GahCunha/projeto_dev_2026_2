      ...
      <header className="flex items-start justify-between gap-4 border-b border-rule/70 p-5">
        <div>
          <p className="mb-1 font-mono text-xs tracking-wider text-ochre uppercase">
            {workshopClass.meetings.length}{' '}
            {workshopClass.meetings.length === 1 ? 'aula' : 'aulas'}
          </p>
      ...
      <div className="flex flex-1 flex-col p-5">
        <ol className="m-0 grid list-none gap-3 p-0">
          {workshopClass.meetings.map((meeting, index) => (
            <li
              ...
              key={meeting.id}
            >
              <span className="grid size-8 place-items-center border border-green font-mono text-xs text-green">
                {index + 1}
              </span>
              <span>
                <strong className="block text-carbon">
        ...
        <dl className="mt-5 grid grid-cols-3 gap-4 border-y border-rule/70 py-4 text-sm">
          ...
          <div>
            ...
            <dd className="mt-1 font-bold text-carbon tabular-nums">
              {workshopClass.occupiedSeats}/{workshopClass.capacity}
            </dd>
          </div>
          <div>
            <dt className="font-mono text-xs tracking-wider text-muted uppercase">
            ...
            <dd
              className={`mt-1 font-bold tabular-nums ${workshopClass.availableSeats === 0 ? 'text-danger' : 'text-success'}`}
            >
              {workshopClass.availableSeats}
            </dd>
          </div>
        </dl>
        ...
        <div className="mt-auto flex flex-wrap items-center gap-x-4 gap-y-3 pt-5">
          ...
          <button
            className="font-mono text-xs font-bold tracking-wider text-blue uppercase underline decoration-saffron decoration-2 underline-offset-4"
            type="button"
            onClick={() => onEdit(workshopClass)}
          >
            Editar
          </button>
          <button
            className={`ml-auto font-mono text-xs font-bold tracking-wider uppercase underline underline-offset-4 ${workshopClass.active ? 'text-danger' : 'text-success'}`}
            type="button"
            onClick={() => onStatusChange(workshopClass)}
          >
            {workshopClass.active ? 'Desativar' : 'Ativar'}
          </button>
        </div>
      </div>
    </article>
