...
export function AdminPagination({ page, totalPages, totalItems, itemLabel, onPageChange }: AdminPaginationProps) {
  if (totalPages <= 1) return null

  return (
    <nav className="mt-5 flex items-center justify-between gap-4" aria-label={`Paginação de ${itemLabel}`}>
      <p className="hidden text-sm text-muted sm:block">{totalItems} {itemLabel} no total</p>
