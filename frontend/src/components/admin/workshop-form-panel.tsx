...
export function WorkshopFormPanel({
  ...
  return (
    ...
    >
      <section
        className="h-full w-full max-w-2xl overflow-y-auto border-l border-rule bg-light shadow-photo"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <form className="space-y-7 px-5 py-6 sm:px-7" onSubmit={handleSubmit}>
          ...
          <div className="border-l-4 border-saffron bg-saffron/10 px-4 py-3 text-sm text-carbon">
            Datas, locais, valores e vagas são configurados nas turmas depois
            que a oficina é criada.
          </div>
          ...
          >
            <textarea
              className={`${inputStyles} min-h-24 resize-y py-3`}
              id="materials"
...
function CoverPreview({
  ...
  return (
    <div className="relative aspect-[16/7] overflow-hidden border border-rule bg-deep shadow-craft-sm">
      ...
      <span className="absolute bottom-3 left-3 bg-carbon/80 px-2 py-1 font-mono text-xs tracking-wider text-paper uppercase">
...
const inputStyles =
  'min-h-11 w-full rounded-sm border border-rule bg-paper px-3 text-sm text-ink placeholder:text-muted/60 hover:border-muted'
