    ...
    <>
      <div className="space-y-3 sm:hidden">
        {enrollments.map((enrollment) => (
          <article
            className="border border-rule bg-paper p-4 shadow-craft-sm"
            key={enrollment.id}
          >
            ...
            <dl className="grid gap-3 border-t border-rule/70 pt-3">
              <div>
                ...
                <dd className="mt-1 text-sm font-bold text-carbon">
            ...
            <EnrollmentActions
              ...
              className="mt-4 border-t border-rule/70 pt-3"
            />
          </article>
        ))}
      </div>
            ...
            <tbody className="divide-y divide-rule/70">
              {enrollments.map((enrollment) => (
                <tr className="transition hover:bg-light" key={enrollment.id}>
                  ...
                  <td className="px-5 py-4 align-middle">
                    <span className="block max-w-xs text-sm font-bold text-carbon">
                      {enrollment.workshop.title}
                    </span>
                    <small className="mt-1 block text-muted">
                      {enrollment.class.name}
                    </small>
                    {!enrollment.workshop.active && (
...
function EnrollmentActions({
  ...
  if (enrollment.status === 'CANCELADA') {
    return (
      <p className={`text-right text-xs text-muted ${className ?? ''}`}>
  ...
  const actionStyles =
    'min-h-10 font-mono text-xs font-bold uppercase tracking-wider underline underline-offset-4 disabled:cursor-wait disabled:opacity-50'
  ...
  return (
    ...
    >
      {enrollment.status === 'PENDENTE' && (
        <>
          <button
            className={`${actionStyles} text-success`}
            type="button"
            disabled={disabled}
            onClick={() => onStatusChange(enrollment, 'CONFIRMADA')}
          >
            Confirmar
          </button>
      ...
      <button
        className={`${actionStyles} text-danger`}
        type="button"
        disabled={disabled}
        onClick={() => onStatusChange(enrollment, 'CANCELADA')}
      >
        Cancelar
      </button>
    </div>
