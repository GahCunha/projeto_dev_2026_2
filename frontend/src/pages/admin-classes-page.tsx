...
export function AdminClassesPage() {
  ...
  const workshopTitle =
    searchParams.get('workshopTitle') ?? 'Oficina selecionada'
  ...
  const [formClass, setFormClass] = useState<WorkshopClass | 'new' | null>(
    isNewWorkshop ? 'new' : null,
  )
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <Link
        className="mb-5 inline-flex min-h-10 items-center font-mono text-xs font-bold tracking-wider text-blue uppercase underline decoration-saffron decoration-2 underline-offset-4"
        to="/admin/oficinas"
      >
        ← Voltar às oficinas
      </Link>
      ...
      {isNewWorkshop && classes.length === 0 && !isLoading && !error && (
        <div
          className="mb-5 border-l-4 border-saffron bg-saffron/10 px-4 py-3 text-sm text-carbon"
          role="status"
        >
          Oficina criada. Cadastre sua primeira turma para definir agenda,
          local, valor e vagas e torná-la visível ao público.
        </div>
      )}
      ...
      {!isLoading && !error && classes.length === 0 && (
        <EmptyState
          title="Esta oficina ainda não possui turmas"
          description="Crie uma turma para definir preço, vagas e as datas das aulas."
          action={
            <Button onClick={() => setFormClass('new')}>
      ...
      {formClass && (
        <ClassFormPanel
          workshopTitle={workshopTitle}
          workshopClass={formClass === 'new' ? undefined : formClass}
          onSubmit={saveClass}
          onClose={closeForm}
        />
      )}
      {statusClass && (
        <ConfirmationDialog
          title={`${statusClass.active ? 'Desativar' : 'Ativar'} “${statusClass.name}”?`}
          description={
            statusClass.active
              ? 'Ela deixará de aparecer na área pública, mas suas inscrições serão preservadas.'
              : 'Ela voltará a aceitar inscrições se ainda possuir aulas futuras e vagas.'
          }
          confirmLabel={statusClass.active ? 'Desativar turma' : 'Ativar turma'}
...
function ClassGridSkeleton() {
  return (
    ...
    >
      {[1, 2].map((item) => (
        <div className="space-y-4 border border-rule bg-paper p-5" key={item}>
          <div className="h-5 w-1/2 animate-loading loading-surface" />
          <div className="h-24 animate-loading loading-surface" />
