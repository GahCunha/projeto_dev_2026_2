...
export function getAdminClasses(workshopId: string, signal?: AbortSignal) {
  return apiRequest<WorkshopClassesResponse>(`/api/admin/oficinas/${workshopId}/turmas`, { signal })
}

export function createAdminClass(workshopId: string, data: ClassFormData) {
  return apiRequest<WorkshopClassResponse>(`/api/admin/oficinas/${workshopId}/turmas`, {
    method: 'POST',
